...
#parameters
tmax = 100000        #maximum number of iterations
visc = 0.001          #kinematic viscosity
Lx = 100            #system size
Ly = 50
uxBC = 0.08        #inlet velocity
...
for t in np.arange(tmax):
    ...
    if((t%100)==0):  #plot the velocity field
        print("time = %d"%t)
        velX = Ux
        velX=np.ma.masked_where( Obst==1, velX)
        im = plt.imshow(velX, interpolation="bilinear", cmap='jet')
